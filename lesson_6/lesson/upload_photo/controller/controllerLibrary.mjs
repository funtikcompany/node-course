...
class LibraryController {
    ...
    static createLibrary(req, res) {
        try {
            ...
            LibraryModulesModel.createLibraryModule(name, author, year, photo)
    ...
    static updateLibrary(req, res) {
        try {
            const id = req.params.id
            const libraryModule = LibraryModulesModel.getLibraryModuleById(id)
            if(libraryModule.photo) {
                deleteFile('uploads', libraryModule.photo)
            }
            const { name, author, year } = req.body
            let photo = null
            if(req.file) {
            ...
            LibraryModulesModel.updateLibraryModule(id, name, author, year, photo)
            res.redirect('/library')
        } catch (error) {
            res.status(500).render('error', { message: error.message,error })
        }
